...
function reasoningEffort(effort) {
    ...
    throw new LlmError('Copilot does not support reasoning effort "' + effort + '"', 'UNSUPPORTED_REASONING_EFFORT');
}
/** Resolve one legal reasoning effort without exposing `off` as a wire value. */
function resolveReasoning(options, defaults) {
...
function dataUrl(data, mediaType) {
    return `data:${mediaType};base64,${Buffer.from(data).toString('base64')}`;
}
async function serializeContent(blocks, resolveImage, signal) {
    const hasImage = blocks.some(block => block.type === 'image');
    if (!hasImage)
        return flattenText(blocks);
...
async function serializeAssistant(message, resolveImage, signal) {
 ...
 */
export function serializeRequest(options, defaults = {}, resolveImage, signal) {
    const messages = [];
